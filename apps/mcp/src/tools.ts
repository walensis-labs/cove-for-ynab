...
export const tools: ToolDef[] = [
  ...
  { name: 'month_close', description: 'READ-ONLY month-close report for a cutoff date (normally the closing month\'s last day): per-credit-card coverage (working & cleared as-of balances vs payment-category available at month end; gap 0 = covered), blockers (unapproved / uncategorized / uncleared before cutoff), overspent (red) categories, and ranked donor categories. Proposes nothing and moves nothing — pair with propose_coverage, then apply approved moves via move_money / assign_budget. gapStatus is \'provisional\' until every blocker is resolved: never present a provisional gap as the final number — resolve blockers (categorize/approve via update_transactions) and re-run.', schema: { plan_id: planId, cutoff: z.string().describe("ISO date cutoff, e.g. '2026-07-31'"), lookback_days: z.number().int().min(1).max(365).optional().describe('straggler scan window, default 120') }, handler: (y, a) => y.monthClose(a.plan_id, { cutoff: a.cutoff, lookbackDays: a.lookback_days }) },
