...
export interface ToolDef {
  ...
  write?: boolean
  /**
   * Truthful Tool Output, Task 3 review fix (IMPORTANT 1): structural replacement for the
   * hand-maintained `MONEY_TOUCHING_TOOL_NAMES` array in test/server.test.ts, which verified only the
   * tools someone remembered to list — `backfill_ledger` shipped money-bearing output without ever
   * being added to it. Set `money: true` on any tool whose output carries a dollar figure or whose
   * schema accepts one (mirrors `write`'s "mark it where the tool is defined" pattern instead of a
   * parallel list that can drift). Non-write money tools (output-emitting) must carry UNIT_NOTE in
   * their description; write money tools document units per-field via `dollars()` (or, where the
   * schema can't carry a per-field description, in prose) — both are enforced by the test derived
   * from this flag.
   */
  money?: boolean
...
export const tools: ToolDef[] = [
  ...
  { name: 'get_plan_overview', description: 'Orient in one call: accounts with balances, current-month Ready to Assign, age of money, category-group totals. Start here.' + UNIT_NOTE, money: true, schema: { plan_id: planId }, handler: (y, a) => y.getPlanOverview(a.plan_id) },
  { name: 'get_month', description: 'One month in full: Ready to Assign, age of money, every category (assigned/activity/available/target status).' + UNIT_NOTE, money: true, schema: { plan_id: planId, month }, handler: (y, a) => y.getMonth(a.plan_id, a.month) },
  // ---- transactions
  { name: 'list_transactions', description: 'List or aggregate transactions with filters (account/category/payee/date/search/amount/unapproved/uncleared), pagination, field selection. aggregate mode returns per-group sums instead of rows — prefer it for spending questions. Output states the effective date window (API defaults to last 365 days).' + UNIT_NOTE, money: true, schema: {
      plan_id: planId,
  ...
  { name: 'get_transaction', description: 'One transaction in full, including split subtransactions.' + UNIT_NOTE, money: true, schema: { plan_id: planId, transaction_id: z.string() }, handler: (y, a) => y.getTransaction(a.plan_id, a.transaction_id) },
  { name: 'create_transactions', description: 'Create one or more transactions (bulk). Supports splits via subtransactions (note: splits cannot be edited after creation — get them right or delete/recreate) and import_id for dedup.', write: true, money: true, schema: {
      plan_id: planId,
  ...
  { name: 'update_transactions', description: 'Bulk edit transactions: categorize, approve, set cleared, edit fields. >5 rows requires confirm:true and expected_count. Undoable.', write: true, money: true, schema: {
      plan_id: planId,
  ...
  { name: 'create_account', description: 'Create an account (checking, savings, cash, creditCard, otherAsset, otherLiability) with a starting balance.', write: true, money: true, schema: { plan_id: planId, name: z.string(), type: z.enum(['checking', 'savings', 'cash', 'creditCard', 'otherAsset', 'otherLiability']), balance: dollars('Starting balance') }, handler: (y, a) => y.createAccount(a.plan_id, { name: a.name, type: a.type, balance: a.balance }) },
  // ---- analytics
  { name: 'spending_summary', description: 'Server-computed spending by category or payee for a window (default last 30 days), optional previous-period comparison with % change. Small output — prefer this over listing rows.' + UNIT_NOTE, money: true, schema: { plan_id: planId, by: z.enum(['category', 'payee']).optional(), since_date: z.string().optional(), until_date: z.string().optional(), compare_to_previous: z.boolean().optional() }, handler: (y, a) => y.getSpendingSummary(a.plan_id, { by: a.by, sinceDate: a.since_date, untilDate: a.until_date, compareToPrevious: a.compare_to_previous }) },
  { name: 'budget_health', description: 'Current-month health check: Ready to Assign, overspent categories, underfunded targets, credit-card payment coverage (float detection).' + UNIT_NOTE, money: true, schema: { plan_id: planId }, handler: (y, a) => y.getBudgetHealth(a.plan_id) },
  { name: 'detect_recurring_charges', description: 'Find recurring charges (subscriptions/bills) from ~13 months of history: cadence, last amount, whether the amount changed.' + UNIT_NOTE, money: true, schema: { plan_id: planId }, handler: (y, a) => y.getRecurringCharges(a.plan_id) },
  { name: 'income_vs_expense', description: 'Monthly income/expense/net series (default 6 months); current month flagged partial.' + UNIT_NOTE, money: true, schema: { plan_id: planId, months: z.number().int().max(24).optional() }, handler: (y, a) => y.getIncomeVsExpense(a.plan_id, { months: a.months }) },
  { name: 'net_worth_history', description: 'Monthly net-worth series computed from full transaction history across all accounts.' + UNIT_NOTE, money: true, schema: { plan_id: planId }, handler: (y, a) => y.getNetWorthHistory(a.plan_id) },
  { name: 'month_close', description: 'READ-ONLY month-close report for a cutoff date (normally the closing month\'s last day): per-credit-card coverage (working & cleared as-of balances vs payment-category available at month end; gap 0 = covered), blockers (unapproved / uncategorized / uncleared before cutoff), overspent (red) categories, and ranked donor categories. Proposes nothing and moves nothing — pair with propose_coverage, then apply approved moves via move_money / assign_budget. gapStatus is \'provisional\' until every blocker is resolved: never present a provisional gap as the final number — resolve uncategorized/unapproved blockers via update_transactions and re-run. unclearedBeforeCutoff rows are register state needing investigation: settled → mark cleared via update_transactions; stale → delete_transaction (user-approved) — a stale entry corrupts workingAsOf.' + UNIT_NOTE, money: true, schema: { plan_id: planId, cutoff: z.string().describe("ISO date cutoff, e.g. '2026-07-31'"), lookback_days: z.number().int().min(1).max(365).optional().describe('straggler scan window, default 120') }, handler: (y, a) => y.monthClose(a.plan_id, { cutoff: a.cutoff, lookbackDays: a.lookback_days }) },
  { name: 'propose_coverage', description: 'READ-ONLY: ordered move proposals to bring every overspent category to zero for the cutoff month — donors first (max 3 donor slices per category) then Ready to Assign, RTA moves tagged source:"rta", anything uncoverable listed in unfundable. Applies nothing: review with the user, then execute approved moves in the returned month via move_money (category→category; moves the delta directly). CAUTION for RTA draws: assign_budget sets the ABSOLUTE assigned amount — first read the category\'s current assigned via get_month, then pass current + move amount; never pass the move amount alone.' + UNIT_NOTE, money: true, schema: { plan_id: planId, cutoff: z.string().describe('same cutoff passed to month_close'), strategy: z.enum(['donors_first', 'rta_only']).optional() }, handler: (y, a) => y.proposeCoverage(a.plan_id, { cutoff: a.cutoff, strategy: a.strategy }) },
  { name: 'get_category_history', description: "READ-ONLY: one category's monthly series (assigned / activity / available) across a month range in a single compact response — use for any single-category trend instead of paging get_month (which returns every category). Months the plan doesn't cover are skipped and listed in skippedMonths. Cost: one API call per month in the range (max 60)." + UNIT_NOTE, money: true, schema: { plan_id: planId, category_id: z.string(), since_month: z.string().describe("first month, 'YYYY-MM'"), until_month: z.string().describe("last month inclusive, 'YYYY-MM'") }, handler: (y, a) => y.getCategoryHistory(a.plan_id, { categoryId: a.category_id, sinceMonth: a.since_month, untilMonth: a.until_month }) },
  { name: 'credit_card_float_history', description: "READ-ONLY: per-month credit-card float analysis over a range — the card's owed balance at each month end (backed out of the current balance) vs its payment category's available, the gap, and changed:true on months where the gap moved (new float appeared or was paid down; a static gap is just carried history). Pass the payment CATEGORY id and the card ACCOUNT id. For a single cutoff with blockers and donor proposals use month_close instead. Cost: ~one API call per month plus two. Each point carries gapChange and direction ('grew' = float increased)." + UNIT_NOTE, money: true, schema: { plan_id: planId, payment_category_id: z.string().describe('the Credit Card Payments category id'), card_account_id: z.string().describe('the credit card account id'), since_month: z.string().describe("'YYYY-MM'"), until_month: z.string().describe("'YYYY-MM' inclusive") }, handler: (y, a) => y.getCreditCardFloatHistory(a.plan_id, { paymentCategoryId: a.payment_category_id, cardAccountId: a.card_account_id, sinceMonth: a.since_month, untilMonth: a.until_month }) },
  { name: 'backfill_ledger', description: "Backfill the LOCAL balance-forward ledger from history (writes ~/.cove/ledger.json only — never touches YNAB): one record per fully-elapsed month for a card with gap, causes, and the discovery summary ('carrying $X since <date>'). Re-runs replace prior backfill records for the same card; real close records are never touched. Run once per card at first setup; costs ~one API call per month. Backfill records key on the card's NAME — if you rename a card in YNAB, re-run backfill (old-name rows remain until replaced)." + UNIT_NOTE, money: true, schema: { plan_id: planId, payment_category_id: z.string().describe('the Credit Card Payments category id'), card_account_id: z.string().describe('the credit card account id'), since_month: z.string().describe("'YYYY-MM'"), until_month: z.string().optional().describe("'YYYY-MM' inclusive; defaults to the current month") }, handler: (y, a) => y.backfillLedger(a.plan_id, { paymentCategoryId: a.payment_category_id, cardAccountId: a.card_account_id, sinceMonth: a.since_month, untilMonth: a.until_month }) },
  // ---- system
  ...
  { name: 'record_month_close', description: 'Writes a LOCAL file only (~/.cove/ledger.json) — never touches YNAB. Persist the balance-forward line at the end of a month-close session: per-card gaps, blocker counts, attributed causes, applied moves with reasons.' + UNIT_NOTE, money: true, schema: {
      plan_id: planId, cutoff: z.string().describe("ISO date cutoff, e.g. '2026-07-31'"), gap_status: z.enum(['provisional', 'final']),
      per_card: z.array(z.object({ account: z.string(), working_as_of: dollars('Working (uncleared-inclusive) balance as-of the cutoff'), cleared_as_of: dollars('Cleared-only balance as-of the cutoff'), available_at_month_end: dollars('Payment category available at month end'), gap: dollars('available_at_month_end minus working_as_of; negative = float') })).min(1),
      blockers: z.object({ unapproved: z.number().int(), uncategorized: z.number().int(), uncleared_before_cutoff: z.number().int() }),
      causes: z.array(z.object({ month: z.string(), change: dollars('Attributed change for the month'), cause: z.string(), narrative: z.string().optional() })).optional(),
      moves: z.array(z.object({ from: z.string(), to: z.string(), amount: dollars('Move amount'), source: z.enum(['category', 'rta']), reason: z.string().optional() })).optional(),
      buffer: dollars('Buffer amount').optional(), note: z.string().optional(),
  ...
  { name: 'get_month_close_ledger', description: "Read past balance-forward records (newest first) — optional cutoff filter. Records come in two kinds: real month-closes are kind:'close' (written by record_month_close); backfill_ledger writes kind:'backfill' history rows. To compare against \"the last close\", filter kind:'close' — otherwise a backfill row (not a real close) may come back newest." + UNIT_NOTE, money: true, schema: { limit: z.number().int().max(50).optional(), cutoff: z.string().optional(), kind: z.enum(['close', 'backfill']).optional() }, handler: async (y, a) => y.getMonthCloseLedger({ limit: a.limit, cutoff: a.cutoff, kind: a.kind }) },
]
