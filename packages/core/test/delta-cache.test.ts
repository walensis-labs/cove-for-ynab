...
describe('DeltaCache', () => {
  ...
  it('never collides across plan/resource pairs containing separator-like characters', () => {
    const c = new DeltaCache()
    c.merge('a:b', 'c', 1, [{ id: 'x' }])
    c.merge('a', 'b:c', 2, [{ id: 'y' }])
    expect(c.knowledge('a:b', 'c')).toBe(1)
    expect(c.knowledge('a', 'b:c')).toBe(2)
    c.invalidate('a')
    expect(c.knowledge('a:b', 'c')).toBe(1)
    expect(c.knowledge('a', 'b:c')).toBeUndefined()
  })
})
