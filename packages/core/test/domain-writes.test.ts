  ...
  it('without a journal: does not mention undo_last, states the move is half-applied, and gives the manual correction', async () => {
    function makeClient() {
      ...
      return { request: vi.fn(async (path: string, opts: any) => {
        // Currency-symbol threading (fix/currency-symbol): moveMoney resolves the plan's real symbol
        // via a /plans fetch before formatting; a resolvable USD plan keeps '$100.00' etc. unchanged.
