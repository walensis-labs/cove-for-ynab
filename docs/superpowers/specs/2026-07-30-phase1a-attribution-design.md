...
2. **Reversal-pair scan** — in the card's transactions within the month ±30 days: find offsetting sets (pairs or +X/−X/+X trios of equal |amount|) whose NET equals `−remaining` (within tolerance) → `payment_reversal`, evidence = the txns. Also categoryless owed-increasing transfers (amount < 0, category_id null, transfer_account_id non-null) summing to ≈ remaining → `uncategorized_debt`.
