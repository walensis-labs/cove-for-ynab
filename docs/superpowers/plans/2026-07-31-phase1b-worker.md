...
4. (Task 4 round) claude.ai custom connectors accept URL-only auth (request headers are beta-gated), so the worker gained a token-in-path route `/mcp/:token` (same constant-time validation, shared handler) — claude.ai uses the URL form; header-capable clients use bearer. The runbook discloses that Workers Logs observability records the token-bearing URL on every request (mitigation documented).
5. (Final-fixes round) The spec's healthy weekly-digest line included a `Buffer: $X` figure; the shipped worker computes no buffer in 1b (there is no Plan/allocation module yet to derive it from) — the healthy one-liner is just "All cards covered." with no buffer figure. Revisit once a Plan module exists to source that number honestly rather than fabricating or omitting it silently.
